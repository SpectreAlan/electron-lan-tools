    ...
    const inter = setInterval(async ()=>{
      ...
      if(!window.loading){
        ...
        setTableData(tableData=>{
          ...
          console.log(columns)
          // @ts-ignore
          window.electron.saveExcel({tableData: [[columns?.[1]?.title || '原始数据', '筛选结果'],...tableData.map(item=>[item.data,item.mark])], title: '数据凑零'})
          return tableData
        })
        clearInterval(inter)
      }
      ...
      <Splitter.Panel defaultSize="40%" min="40%" max="50%">
        <div className={'formBox'}>
          ...
          <Result
            ...
            title={loading ? '数据筛选中...' : (!result.total ? '等待上传':  '匹配结果')}
            subTitle={result.selected ? <div>
              <div>共筛选出 <span className={'green'}>{result.selected}</span> 条数据满足条件项目</div>
              <div>剩余 <span className={'red'}>{result.total - result.selected}</span>条</div>
